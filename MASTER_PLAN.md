...
### Phase 3 · Retrieval & Knowledge (weeks 10–12)

#### T5 · Vector Database (HNSW index)
**Build:** **hnswlite** — an HNSW approximate-nearest-neighbor index from the paper: multi-layer skip-list-like graph, `efConstruction`/`efSearch`, M-neighbor selection heuristic, insert + search + delete (tombstones), persistence to disk. **Ladder:** (1) brute-force cosine search baseline + recall harness → (2) navigable small world graph, single layer → (3) layered HNSW with level sampling → (4) neighbor-selection heuristic vs naive-closest (measure the difference) → (5) persistence + memory-mapped load → (6) benchmark recall@10 vs QPS against hnswlib on 100k finance embeddings; plot the tradeoff curve. **Done when:** ≥0.9 recall@10 at ≥10× brute-force QPS on 100k vectors. **Videos:** 4 eps — why exact search dies; skip lists → NSW → HNSW (the flagship animated-graph episode, very slow); the insert algorithm; tuning ef/M with live plots. **AlphaDesk:** the desk's vector store for filings/news/research chunks. **Effort:** ~16 h. *Ref: Malkov & Yashunin 2016.*

#### T51 · Database driver (for vectors)
**Build:** a proper client driver for hnswlite: a wire protocol (length-prefixed msgpack over TCP), a server daemon wrapping the index, and a Python client with connection pooling, retries with jitter, timeouts, batched upserts, and a DB-API-flavored interface — the "boring" engineering that makes T5 production-shaped. **Ladder:** (1) protocol spec + framing codec with fuzz tests → (2) asyncio server around hnswlite → (3) client: pool, retry, timeout budget → (4) batching + backpressure → (5) chaos test: kill the server mid-stream, verify client behavior. **Done when:** 10k concurrent queries survive a server restart with correct error semantics and no hangs. **Videos:** 2 eps — what a driver actually does (told from your OMS connectivity experience); framing, pooling, retries. **AlphaDesk:** all services talk to the vector store through this driver. **Effort:** ~10 h.

#### T6 · RAG pipeline
**Build:** retrieval-augmented generation end-to-end: chunking strategies (fixed/recursive/structure-aware for filings), embedding + indexing, hybrid retrieval (BM25 from scratch + vectors, reciprocal-rank fusion), cross-encoder reranking, context assembly with citations, and faithfulness checks. **Ladder:** (1) BM25 implemented from the formula, tested vs rank_bm25 → (2) chunkers + eval set of 100 finance Q-A pairs with gold passages → (3) hybrid fusion, measure hit-rate lift → (4) reranker (small cross-encoder) → (5) generation with mandatory citation spans → (6) RAG triad metrics (context relevance / groundedness / answer relevance) wired into T27's harness. **Done when:** measured hit-rate and groundedness improvements at each ladder step — the pipeline is a chart, not a vibe. **Videos:** 4 eps — why context beats fine-tuning for facts; BM25 slowly; hybrid + rerank; hallucination and groundedness. **AlphaDesk:** "Ask the desk about any filing" — the research copilot's backbone. **Effort:** ~14 h.

#### T36 · Semantic Router
**Build:** an intent router that sends each desk query to the right pipeline (RAG / text-to-SQL / agent / small-talk / refuse): embedding-similarity routes with calibrated thresholds, a trained linear classifier head on your T43 embeddings, confidence + fallback logic, and an eval set with confusion matrix. **Ladder:** (1) route schema + 300-utterance labeled set (synthetic + hand-edited) → (2) cosine-to-centroid router → (3) trained softmax head; compare → (4) threshold calibration (reliability diagram) → (5) shadow-mode logging in AlphaDesk. **Done when:** ≥95% routing accuracy on held-out set with calibrated abstention. **Videos:** 2 eps — routing as retrieval; calibration (what confidence actually means). **AlphaDesk:** the front door of the desk UI. **Effort:** ~8 h.

#### T37 · Knowledge Graph builder
**Build:** an information-extraction pipeline that turns filings/news into a typed property graph: entity extraction (issuers, tickers, people, instruments), relation extraction via SLM-with-schema prompting + rule fallbacks, entity resolution/dedup, storage in SQLite with a tiny Cypher-ish query layer, and graph visualizations. **Ladder:** (1) schema design (node/edge types for capital markets) → (2) NER via patterns + model, measured F1 on a labeled slice → (3) relation extraction with constrained JSON output (uses T25's parser!) → (4) entity resolution (blocking + similarity) → (5) query layer + NetworkX analytics (centrality of issuers in news flow). **Done when:** 500-doc corpus yields a graph where spot-checked edges are ≥90% correct. **Videos:** 3 eps — documents to graphs; extraction with schemas; entity resolution (the unsung hard part). **AlphaDesk:** the issuer/supplier/event graph behind GraphRAG and risk queries. **Effort:** ~12 h.

#### T20 · Graph RAG system
**Build:** retrieval that walks the knowledge graph: query → seed entities → k-hop subgraph expansion with edge-type weighting → community summaries (Leiden clustering + SLM-written summaries, GraphRAG-style) → context assembly mixing graph paths with vector chunks; head-to-head eval vs plain RAG on multi-hop questions. **Ladder:** (1) multi-hop eval set ("which portfolio names share a supplier that guided down?") → (2) seed-entity linking → (3) subgraph expansion + path serialization → (4) community detection + hierarchical summaries → (5) hybrid assembly; A/B vs T6 with the T27 harness. **Done when:** GraphRAG measurably beats plain RAG on multi-hop questions and loses on single-fact ones — and you can explain why. **Videos:** 3 eps — where vector RAG fails; walking the graph; community summaries. **AlphaDesk:** cross-holding/supply-chain risk answers. **Effort:** ~12 h. *Ref: Microsoft GraphRAG 2024.*

#### T41 · Text-to-SQL engine
**Build:** natural language → SQL over the AlphaDesk market DB (DuckDB: prices, positions, trades, fundamentals): schema linking, few-shot prompt construction, constrained decoding of SQL via your T25 grammar, execution with sandboxed read-only connection, self-correction loop on error, and an accuracy eval (execution-match on 150 hand-built finance questions). **Ladder:** (1) schema card generation + question set → (2) baseline prompting, measure execution accuracy → (3) schema linking (retrieve relevant tables/columns) → (4) grammar-constrained SQL generation → (5) error-feedback retry loop → (6) guard: read-only + row limits + cost caps. **Done when:** ≥80% execution accuracy on the eval set, zero write statements ever executed (property-tested). **Videos:** 3 eps — why NL→SQL is retrieval + constraints; schema linking; constrained decoding meets self-repair. **AlphaDesk:** "What's my exposure to IT sector vs last month?" answered from the DB. **Effort:** ~12 h.

#### T50 · Feature Store
**Build:** a mini feature store for trading signals: offline store (Parquet + DuckDB) and online store (Redis-compatible dict server you write), feature definitions as code (decorated Python functions), point-in-time-correct training-set assembly (the leakage killer), scheduled materialization, and drift monitoring. **Ladder:** (1) feature registry + definitions (returns, vol, RSI, news-sentiment from your models) → (2) offline materialization with backfill → (3) point-in-time join, with a deliberately-leaky version first, measured "too good" model as the lesson → (4) online store + get_online_features latency budget → (5) drift stats (PSI) dashboard. **Done when:** PIT-correct dataset reproduces exactly across runs; the leaky version is demonstrably fraudulent. **Videos:** 3 eps — features are a data-management problem; point-in-time correctness (the flagship "financial ML honesty" episode); online serving + drift. **AlphaDesk:** every model input flows through it. **Effort:** ~12 h.

#### T42 · Recommendation system (Two-tower architecture)
**Build:** a two-tower retrieval model recommending research items (filings, news, notes) to desk users: user tower (position/watchlist/history features from T50) + item tower (T43 embeddings + metadata), in-batch softmax with logQ correction, ANN serving through hnswlite, offline metrics + a tiny online simulator. **Ladder:** (1) implicit-feedback dataset from synthetic desk logs → (2) towers + in-batch negatives → (3) logQ/popularity correction, measure lift → (4) export item tower to hnswlite; end-to-end retrieval latency → (5) offline Recall@k/NDCG + simulated CTR replay. **Done when:** beats popularity and pure-content baselines on Recall@20 with the serving path under a latency budget. **Videos:** 3 eps — recsys as retrieval; the two towers slowly; serving embeddings. **AlphaDesk:** the "For your desk this morning" panel. **Effort:** ~12 h. *Ref: YouTube two-tower papers, sampling-bias-corrected retrieval (Yi 2019).*

### Phase 4 · Agents & Safety (weeks 13–15)

#### T1 · Reasoner (Chain of Thought)
**Build:** a reasoning layer, not a prompt: CoT prompting harness with self-consistency (sample k chains, vote), a verifier model scoring chains, best-of-n selection, and a benchmark showing when CoT helps vs hurts (arithmetic/tabular/finance word problems), plus budget-aware "think less on easy inputs" routing. **Ladder:** (1) eval set (GSM8K-slice + finance word problems + tabular questions) → (2) direct vs CoT baseline, measured → (3) self-consistency with k ablation → (4) verifier-scored best-of-n → (5) adaptive budget policy. **Done when:** accuracy-vs-token-budget curves are produced and the adaptive policy dominates fixed policies. **Videos:** 3 eps — what CoT actually does (with honest limits); self-consistency; verification & budgets. **AlphaDesk:** the morning-brief generator's reasoning engine (positions + news → thesis with checkable steps). **Effort:** ~10 h. *Ref: Wei 2022, Wang 2022 (self-consistency).*

#### T2 · Agent loop (ReAct pattern)
**Build:** a from-scratch agent runtime: Thought→Action→Observation loop, a typed tool registry, scratchpad memory, max-step + cost budgets, retry/reflection on tool errors, trajectory logging + replay viewer (HTML), and multi-turn task eval. **Ladder:** (1) the bare loop with a calculator + market-data tool → (2) typed tool schemas + argument validation → (3) error reflection ("observation: your SQL failed because…") → (4) budgets, loop-detection, graceful abort → (5) trajectory store + replay page → (6) eval on 20 scripted desk tasks with success criteria. **Done when:** ≥80% task success with zero infinite loops across the eval suite; every failure is replayable. **Videos:** 4 eps — the loop is embarrassingly simple (build it live, slowly); tools as contracts; failure & reflection; reading trajectories. **AlphaDesk:** the desk research agent — the program's centerpiece demo. **Effort:** ~14 h. *Ref: Yao 2022 (ReAct).*

#### T24 · Function Calling router
**Build:** the layer that turns model text into safe tool invocations: JSON-schema tool specs, few-shot + grammar-constrained argument generation (reusing T25), parallel call planning, argument validation with typed errors back to the model, timeouts/circuit breakers per tool, and an audit log. **Ladder:** (1) tool spec registry (screener, quotes, backtester, order-sim, filings-search) → (2) call extraction with grammar enforcement → (3) validation + coercion + typed error returns → (4) parallel dispatch with dependency detection → (5) circuit breakers + audit trail → (6) accuracy eval: 100 utterances → correct tool + args. **Done when:** ≥95% correct tool selection with 100% schema-valid arguments (the grammar makes invalid JSON impossible — property-tested). **Videos:** 2 eps — from text to a safe call; breakers, audits, parallelism. **AlphaDesk:** the ReAct agent's hands; also the OMS-sim gateway. **Effort:** ~10 h.

#### T18 · Code interpreter sandbox
**Build:** a sandboxed Python execution service for agent-generated quant code: subprocess isolation with rlimits (CPU/memory/time), no-network namespace, AST-based import/API allowlisting, filesystem jail (tmpdir), stdout/plot capture (matplotlib → PNG), session state via pickled namespace, and an escape-attempt test suite. **Ladder:** (1) naive exec + why it's terrifying (demonstrated safely) → (2) subprocess + resource limits → (3) AST allowlist walker with bypass tests → (4) tmpdir jail + artifact capture → (5) stateful sessions → (6) red-team suite: 25 escape attempts, all contained. **Done when:** the escape suite passes; a fork-bomb, an `open('/etc/passwd')`, and an exfil attempt all die cleanly with useful errors. **Videos:** 3 eps — why agents need a padded room; building the jail layer by layer; attacking your own sandbox. **AlphaDesk:** the agent's backtesting/analysis notebook executor. **Effort:** ~12 h.

#### T28 · Guardrails system (Input/Output filtering)
**Build:** a policy engine wrapping every AlphaDesk model call: input filters (prompt-injection heuristics + a trained classifier on your embeddings, PII detection, off-topic deflection), output filters (financial-advice disclaimer policy, restricted-list mentions, order-ticket pre-trade checks: position limits, notional caps, restricted instruments), a policy DSL (YAML rules → compiled checks), decision logging, and a bypass-rate eval. **Ladder:** (1) threat model + policy inventory for a trading desk → (2) rule engine + YAML DSL → (3) injection classifier trained on public injection datasets + your synthetics → (4) pre-trade compliance checks as pure functions with property tests → (5) layered pipeline + decision audit log → (6) eval: attack corpus bypass rate, false-positive rate on legit queries. **Done when:** <5% bypass on the attack corpus at <2% false positives; every pre-trade rule has exhaustive property tests. **Videos:** 3 eps — guardrails as compliance engineering (your home turf); building the policy engine; measuring safety honestly. **AlphaDesk:** wraps everything; the compliance story in your demo. **Effort:** ~12 h.

#### T27 · LLM Eval harness
**Build:** a lm-eval-harness-shaped framework you own: task registry (YAML task specs), deterministic prompting + seeding, metrics (exact match, F1, perplexity, LLM-as-judge with agreement analysis), result caching keyed by config-hash, regression gates for CI, HTML report cards — plus the **AlphaDesk domain suite**: finance QA, filings summarization faithfulness, text-to-SQL execution match, routing accuracy, agent task success. **Ladder:** (1) task spec format + 3 seed tasks → (2) runner with caching + seeds → (3) judge protocol with human-agreement spot-check → (4) domain suite wiring (imports evals from T6/T36/T41/T2) → (5) regression gate + report card generation. **Done when:** one command evaluates any model/version across the suite and diffs against the last run; a deliberate regression is caught. **Videos:** 3 eps — evals are the product spec; building the runner; judging the judge. **AlphaDesk:** the quality dashboard every phase reports into — built early, used forever. **Effort:** ~12 h.

#### T46 · Adversarial attack generator
**Build:** a red-team toolkit aimed at your own stack: prompt-injection corpus generator (mutation + templates), GCG-style greedy token-su
