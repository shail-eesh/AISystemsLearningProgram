...
```props
{"eyebrow": "mistake 2", "title": "Evaluating on overlapping random windows", "kind": "bar", "bars": [{"label": "random\ndraw 1", "value": 1.0395}, {"label": "random\ndraw 2", "value": 1.0724}, {"label": "random\ndraw 3", "value": 0.9903}, {"label": "random\ndraw 4", "value": 1.0658}, {"label": "sequential\nany run", "value": 1.0649, "color": "#5ac48c"}], "yLabel": "held-out loss", "caption": "Same model, same data. Random windows overlap and skip; sequential windows walk the split once, in order."}
```

Mistake two: evaluating on randomly sampled windows. Random draws overlap, so some tokens get counted several times and others not at all, and the number wobbles between runs of the same model on the same data. Here are four random draws from one model: one point zero four, one point zero seven, zero point nine nine, one point zero seven. A spread of eight hundredths of a nat, purely from which windows happened to come up — which is five times the entire margin between our smallest and largest model. Now the green bar: sequential evaluation, walking the split once in non-overlapping windows. It is deterministic — we ran it five times and got one point zero six four nine, five times, to every decimal place. Random windows are the right thing for *training*, where you want variety. They are the wrong thing for a number you are going to put in a table and compare against another number.
